...
public class SipHome extends SherlockFragmentActivity implements
	...
	private void startSipService() {
		Thread t = new Thread("StartSip") {
			public void run() {
				...
				// postStartSipService(); //khan-fingi: i dont want it to show
				// account or anything else autometically..always open the
				// dialer
			};
		};
		t.start();
	...
	// khan-fingi: gets the number of sip accounts. if no account is created,
	// then checkin is not complete
	public static int getSipAccountCount(ContentResolver cr) {
		Cursor c = cr.query(SipProfile.ACCOUNT_URI,
	...
	// khan-fingi: returns the account basic information of display name and account_id .
	public static List<HashMap<String, String>> getSipAccountInformation(ContentResolver cr) {

	        Cursor c = cr.query(SipProfile.ACCOUNT_URI,
	                DBProvider.ACCOUNT_FULL_PROJECTION, null, null, null);
	        List<HashMap<String, String>> accountData = new LinkedList<HashMap<String, String>>();
	        if (c != null) {
	            try {
	                while (c.moveToNext()) {
	                    SipProfile account = new SipProfile(c);
	                    Map<String, String> map = new HashMap<String, String>();
	                    map.put("displayName", account.display_name);
	                    map.put("accountId", account.acc_id);
	                    accountData.add((HashMap<String, String>) map);
	                   // SipProfile x = account;
	                }
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case CLOSE_MENU:
			...
			if (prefProviderWrapper.isValidConnectionForIncoming()) {
				...
				ArrayList<String> networks = prefProviderWrapper
						.getAllIncomingNetworks();
				if (networks.size() > 0) {
					String msg = getString(
							R.string.disconnect_and_will_restart,
